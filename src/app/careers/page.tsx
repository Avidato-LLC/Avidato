    ...
    <main className="max-w-4xl mx-auto py-20 px-4 bg-[#f6f6f6] min-h-screen">
      ...
      <div className="flex flex-col gap-8">
        {careers.map((role, idx) => (
          <div key={idx} className="border rounded-xl bg-white p-8 shadow flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-2xl font-bold mb-2">{role.title}</h2>
              <p className="mb-3 text-gray-700">{`We're looking for talented ${role.title}${role.title.endsWith('s') ? 'es' : 's'} to join our team.`}</p>
            ...
            <div className="flex-shrink-0 mt-4 md:mt-0 md:ml-8">
              <Link href="https://form.typeform.com/to/almI6bRO" target="_blank" className="inline-flex items-center px-5 py-2 bg-black text-white font-semibold rounded-lg hover:bg-gray-800 transition">
                Apply
